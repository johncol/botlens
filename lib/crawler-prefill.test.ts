...
import { getCrawlerComparisonInitialValues } from "./crawler-prefill";

describe("getCrawlerComparisonInitialValues", () => {
  it("maps development environment variables to initial values", () => {
    expect(
      getCrawlerComparisonInitialValues({
  ...
  it("leaves missing development variables blank", () => {
    expect(
      getCrawlerComparisonInitialValues({ NODE_ENV: "development" }),
    ).toEqual({
      domain: "",
      page: "",
      credentials: {
