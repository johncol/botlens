...
describe("compareMarkdowns", () => {
  ...
  describe("non-strict mode", () => {
    describe("video/poster link equivalency", () => {
    ...
    describe(".qa subdomain URL equivalency", () => {
      it("treats URLs differing only by a .qa subdomain as equal", () => {
        const human =
          "![Alt](https://assets.aritzia.com/image/upload/q_auto,f_auto/image.jpg)\n";
        const crawler =
          "![Alt](https://assets.qa.aritzia.com/image/upload/q_auto,f_auto/image.jpg)\n";
        const result = compareMarkdowns(human, crawler, false);
        expect(result.isIdentical).toBe(true);
      });

      it("treats .qa subdomain as equal in both directions", () => {
        const withQa =
          "![Alt](https://assets.qa.aritzia.com/image/upload/q_auto,f_auto/image.jpg)\n";
        const withoutQa =
          "![Alt](https://assets.aritzia.com/image/upload/q_auto,f_auto/image.jpg)\n";
        const result = compareMarkdowns(withQa, withoutQa, false);
        expect(result.isIdentical).toBe(true);
      });

      it("still detects differences beyond a .qa subdomain", () => {
        const human =
          "![Alt](https://assets.aritzia.com/image/upload/image-a.jpg)\n";
        const crawler =
          "![Alt](https://assets.qa.aritzia.com/image/upload/image-b.jpg)\n";
        const result = compareMarkdowns(human, crawler, false);
        expect(result.isIdentical).toBe(false);
      });

      it("strict=true still treats .qa subdomain URLs as different", () => {
        const human =
          "![Alt](https://assets.aritzia.com/image/upload/image.jpg)\n";
        const crawler =
          "![Alt](https://assets.qa.aritzia.com/image/upload/image.jpg)\n";
        const result = compareMarkdowns(human, crawler, true);
        expect(result.isIdentical).toBe(false);
      });
    });
  });
